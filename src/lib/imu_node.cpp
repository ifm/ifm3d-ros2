...
ImuNode::ImuNode(const std::string& node_name, const rclcpp::NodeOptions& opts)
...
TC_RETVAL ImuNode::on_configure(const rclcpp_lifecycle::State& prev_state)
{
  ...
  //
  this->modules_.push_back(this->imu_module_);
...
TC_RETVAL ImuNode::on_activate(const rclcpp_lifecycle::State& prev_state)
{
  ...
  this->fg_->OnNewFrame(std::bind(&ImuNode::frame_callback, this, std::placeholders::_1));
  this->fg_->OnError(std::bind(&ImuNode::error_callback, this, std::placeholders::_1));

  this->fg_diag_->OnAsyncError(
      std::bind(&ImuNode::async_error_callback, this, std::placeholders::_1, std::placeholders::_2));
  this->fg_diag_->OnAsyncNotification(
      std::bind(&ImuNode::async_notification_callback, this, std::placeholders::_1, std::placeholders::_2));
...
TC_RETVAL ImuNode::on_deactivate(const rclcpp_lifecycle::State& prev_state)
{
...
TC_RETVAL ImuNode::on_cleanup(const rclcpp_lifecycle::State& prev_state)
{
...
TC_RETVAL ImuNode::on_shutdown(const rclcpp_lifecycle::State& prev_state)
{
...
TC_RETVAL ImuNode::on_error(const rclcpp_lifecycle::State& prev_state)
{
...
void ImuNode::init_params()
{
  ...
  config_file_descriptor.description =
      "Path to an O3R JSON configuration file applied during CONFIGURE. If empty, the device configuration is not modified.";
  ...
  pcic_port_descriptor.description =
      ...
      "is connected to.";
  this->declare_parameter("pcic_port", 50016, pcic_port_descriptor);
...
{
  this->imu_module_->handle_frame(frame);

  RCLCPP_DEBUG(this->logger_, "Frame callback done.");
}

void ImuNode::error_callback(const ifm3d::Error& error)
{
  RCLCPP_ERROR(logger_, "Error received from ifm3d: %s", error.what());
  // TODO send diagnostic message
}

void ImuNode::async_error_callback(int i, const std::string& s)
{
  this->diag_module_->handle_error(i, s);
  RCLCPP_DEBUG(this->logger_, "Async error callback done.");
}

void ImuNode::async_notification_callback(const std::string& s1, const std::string& s2)
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(ifm3d_ros2::ImuNode)
