...
PdsNode::PdsNode(const rclcpp::NodeOptions& opts) : PdsNode::PdsNode("pds_node", opts)
{
}

PdsNode::PdsNode(const std::string& node_name, const rclcpp::NodeOptions& opts)
...
TC_RETVAL PdsNode::on_configure(const rclcpp_lifecycle::State& prev_state)
{
  ...
  //
  this->modules_.push_back(this->pds_module_);
...
TC_RETVAL PdsNode::on_activate(const rclcpp_lifecycle::State& prev_state)
{
  ...
  this->fg_->OnNewFrame(std::bind(&PdsNode::frame_callback, this, std::placeholders::_1));
  this->fg_->OnError(std::bind(&PdsNode::error_callback, this, std::placeholders::_1));

  this->fg_diag_->OnAsyncError(
      std::bind(&PdsNode::async_error_callback, this, std::placeholders::_1, std::placeholders::_2));
  this->fg_diag_->OnAsyncNotification(
      std::bind(&PdsNode::async_notification_callback, this, std::placeholders::_1, std::placeholders::_2));
  ...
  buffer_list.push_back(ifm3d::buffer_id::O3R_RESULT_JSON);
...
TC_RETVAL PdsNode::on_deactivate(const rclcpp_lifecycle::State& prev_state)
{
...
TC_RETVAL PdsNode::on_cleanup(const rclcpp_lifecycle::State& prev_state)
{
...
TC_RETVAL PdsNode::on_shutdown(const rclcpp_lifecycle::State& prev_state)
{
...
TC_RETVAL PdsNode::on_error(const rclcpp_lifecycle::State& prev_state)
{
...
void PdsNode::init_params()  // TODO cleanup params
{
  ...
  config_file_descriptor.description =
      "Path to an O3R JSON configuration file applied during CONFIGURE. If empty, the device configuration is not "
      "modified.";
  this->declare_parameter(config_file_descriptor.name, "", config_file_descriptor);
  ...
  rcl_interfaces::msg::ParameterDescriptor app_instance_descriptor;
  app_instance_descriptor.name = "app_instance";
...
{
  this->pds_module_->handle_frame(frame);

  RCLCPP_DEBUG(this->logger_, "Frame callback done.");
}

void PdsNode::error_callback(const ifm3d::Error& error)
{
  RCLCPP_ERROR(logger_, "Error received from ifm3d: %s", error.what());
  // TODO send diagnostic message
}

void PdsNode::async_error_callback(int i, const std::string& s)
{
  this->diag_module_->handle_error(i, s);
  RCLCPP_DEBUG(this->logger_, "Async error callback done.");
}

void PdsNode::async_notification_callback(const std::string& s1, const std::string& s2)
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(ifm3d_ros2::PdsNode)
